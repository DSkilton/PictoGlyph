...
class IngestionReviewItemServiceTest {

	@Mock
	private IngestionReviewItemRepository repository;
	...
	@BeforeEach
	void setUp() {
		service = new IngestionReviewItemService(repository, datasetPreparationService);
	}
	...
	void shouldRevalidateDatasetAfterReviewDismissal() {
		...
		when(repository.findById(55L)).thenReturn(Optional.of(reviewItem));
		when(repository.save(reviewItem)).thenReturn(reviewItem);

		UpdateIngestionReviewItemRequest request = new UpdateIngestionReviewItemRequest(IngestionReviewStatus.DISMISSED, "Item reviewed and intentionally excluded");
